...
import torchvision
from rnop_dataset import RNOPDataset, PATHOLOGIES, ViewType
...
SAVE_DIR: str = "./out"
SAVE_PATTERN: str = r"rnop_(frontal|lateral)_p[0-9]{2}_e[0-9]{3}.pt"


def get_save_filename(view_type: ViewType, pi: int, ei: int) -> str:
    return f"rnop_{view_type}_p{pi:02d}_e{ei:03d}.pt"
...
def main(argv: List[str]) -> None:
    ...
    header: str = f"RNOP v2 Model for Pathology {pi:02d} ({PATHOLOGIES[pi]}) -- {str(view_type).capitalize()} View"
    print(f"{header}\n{'=' * len(header)}\n", flush=True)
    ...
    data_loader = DataLoader(
        RNOPDataset(view_type, pi, device), batch_size=BATCH_SIZE, shuffle=True
    )
