...
import torchvision
from adnop_dataset import ADNOPDataset, PATHOLOGIES, ViewType
...
def get_save_filename(view_type: ViewType, pi: int, ei: int) -> str:
    return f"adnop_{view_type}_p{pi:02d}_e{ei:03d}.pt"
...
def main(argv: List[str]) -> None:
    ...
    header: str = f"ADNOP Model for Pathology {pi:02d} ({PATHOLOGIES[pi]}) -- {str(view_type).capitalize()} View"
    print(f"{header}\n{'=' * len(header)}\n", flush=True)
    ...
    data_loader = DataLoader(
        ADNOPDataset(view_type, pi, device), batch_size=BATCH_SIZE, shuffle=True
    )
